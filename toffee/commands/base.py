...
class BaseCommand:
    ...
    def validate_environment(self, env_name: str) -> bool:
        ...
        if not valid:
            ...
            if "not found" in error_msg:
                ...
                if suggestion:
                    error_console.print(f"[yellow]Did you mean:[/] [bold]{suggestion}[/]?")
    ...
    def display_environments(self) -> None:
        ...
        if not env_names:
            console.print("[yellow]No environments found.[/] Make sure you have .tfvars files in the vars/ directory.")
            return
        ...
        for name in env_names:
            ...
            status_style = "green" if vars_exists and backend_exists else "red"
    ...
    def execute_terraform_command(self, env_name: str, command_name: str, extra_args: List[str] = None) -> int:
        """
        ...
        # Run command
        return_code, stdout, stderr = self.terraform.run_command(command_name, env, extra_args)
        ...
        if return_code == 0:
            console.print(f"[bold green]Command succeeded[/]")
        else:
            error_console.print(f"[bold red]Command failed with exit code {return_code}[/]")
